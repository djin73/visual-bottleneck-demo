...
function search(parameters, datasets_list) {
  ...
  Object.entries(datasets_list).forEach(([dset_name, bottlenecks]) => {
    ...
    bottlenecks.forEach((bot_name) => {
      if (
        !parameters.search ||
        bot_name.fuzzy(parameters.search) ||  
        dset_name.fuzzy(parameters.search) 
        // bot_name.toLowerCase().includes()
      ) {
...
$(document).ready(function () {
  $.get("/get-random-dataset-bottleneck", ({ data, success }) => {
    if (success) {
      ...
      console.error(data);
      alert(
        "An error occurred while trying to retrieve a random bottleneck; try again."
      );
    }
  });
});
